...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (system_timer && timer_elapsed32(system_timer) > 3000) {
        hs_system_switch();
        system_timer = 0;
    }
...
void hs_system_switch(void) {
    ...
    if (keymap_is_base_layer()) {   
        mac_status = keymap_is_mac_system();
        if (mac_status) { 
            hs_rgb_blink(HS_RGB_SYSTEM_SWITCH_INDEX, 0x00, 0x00, 0xff);
            set_single_persistent_default_layer(_BL);               
            layer_move(0); 
        } else { 
            hs_rgb_blink(HS_RGB_SYSTEM_SWITCH_INDEX, 0xff, 0xff, 0xff);                 
            set_single_persistent_default_layer(_MBL);               
            layer_move(0);
        }
    } else {    
...
void hs_rgb_blink(uint8_t hs_index,uint8_t r,uint8_t g,uint8_t b) {
    rgb_matrix_hs_set_remain_time(hs_index, 0);
    rgb_matrix_hs_indicator_set(hs_index, (RGB){r, g, b}, 250, 3);
}
...
void query(void){
    if (rk_bat_req_flag) {
        ...
        if(*md_getp_bat() > 80  && *md_getp_bat() <= 100){
        ...
        }else if(*md_getp_bat() > 60 && *md_getp_bat() <= 80){
            rgb_matrix_set_color(5, RGB_GREEN);
            rgb_matrix_set_color(4, RGB_GREEN);
            rgb_matrix_set_color(3, RGB_GREEN);
            rgb_matrix_set_color(2, RGB_GREEN);
            rgb_matrix_set_color(1, 0x00,0x00,0x00);
        }else if(*md_getp_bat() > 40 && *md_getp_bat() <= 60) {
            rgb_matrix_set_color(5, RGB_GREEN);
            rgb_matrix_set_color(4, RGB_GREEN);
            rgb_matrix_set_color(3, RGB_GREEN);
            rgb_matrix_set_color(2, 0x00,0x00,0x00);
            rgb_matrix_set_color(1, 0x00,0x00,0x00);
        }else if(*md_getp_bat() > 20 && *md_getp_bat() <= 40) {
...
void hs_light_choose(void) {
    if(hs_right_flag == 1 && hs_left_flag == 0){    
        for (uint8_t i = 11; i <= 15; i++) {
            rgb_matrix_set_color(i, 0x00, 0x00, 0x00);
        }
    }else if(hs_right_flag == 0 && hs_left_flag == 1){
